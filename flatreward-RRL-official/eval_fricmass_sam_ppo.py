...
def main(args):
    ...
    print("env_evaluate:", env_evaluate.unwrapped.model.body_names)

    if args.env in ['HalfCheetah-v3', 'Walker2d-v3', 'Hopper-v3', 'Ant-v3', 'Swimmer-v3', 'InvertedPendulum-v2']:
        if args.env == 'HalfCheetah-v3':
            ...
            fric_fractions = np.linspace(0.4, 1.6, 11)  
            fric_bodies = ['front_left_leg', 'front_right_leg']  #, 'front_right_leg', 'back_left_leg', 'back_right_leg'
            mass_fractions = np.linspace(0.5, 1.5, 11)  
            mass_bodies = ['torso']
            ...
            # Define friction and mass parameters for InvertedPendulum
            fric_fractions = np.linspace(0.2, 1.8, 11)  
            fric_bodies = ['cart']  
            mass_fractions = np.linspace(0.2, 1.8, 11)  
            mass_bodies = ['pole']

        # Retrieve original mass and friction values
        print("\n=== Retrieving Original Mass and Friction Values ===")
        print(env_evaluate.unwrapped.model.body_names)
        print(env_evaluate.unwrapped.model.body_mass)
        print(env_evaluate.unwrapped.model.geom_names)
        print(env_evaluate.unwrapped.model.geom_friction)
        print("=========================================")
        original_masses = {}
